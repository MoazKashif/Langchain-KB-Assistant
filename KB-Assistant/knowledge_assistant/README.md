...
A small LangChain application that answers questions using a local knowledge base.
It demonstrates **model integration, prompt design, structured output, tools,
retrieval, logging, and basic evaluation**.
...
`needs_human_review=true` and `confidence=0.0`.
...
- **Never trust the model's `sources` field directly.** It repeatedly returned
  keywords instead of document names, so we reconcile `sources` deterministically
  from `matched_records` against the knowledge base.
...
- **CSV hygiene matters.** A comma inside an unquoted CSV field silently shifts
  ...
  proper CSV quoting.
- **Confidence calibration.** `confidence` is model-judged; we found Groq's
  Llama-3.3 to be well calibrated on this dataset but it is not verified
